...
class Customer:
	...
	def statement(self):
		...
		for movie,daysRented in self.rental.items():
			this_amount = 0
			each = Rental(movie,daysRented)

			if each.get_movie().get_priceCode()== Movie.REGULAR:
				this_amount+=2
				if(each.get_daysRented()>2):
					this_amount+=(each.get_daysRented() - 2)*1.5
				
			elif each.get_movie().get_priceCode()== Movie.NEW_RELEASE:
				this_amount+=each.get_daysRented() * 3.0
				
			elif each.get_movie().get_priceCode()== Movie.CHILDREN:
				this_amount+=1.5
				if(each.get_daysRented()>3):
					this_amount+=(each.get_daysRented() - 3)*1.5
				

			frequent_renter_points+=1
			if each.get_movie().get_priceCode() == Movie.NEW_RELEASE and each.get_daysRented()>1:
				frequent_renter_points+=1
